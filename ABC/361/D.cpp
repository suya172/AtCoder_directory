...
--------------------------------------------------------
*/
//パズルの最小手順->状態数が十分少ない->BFS
//状態数O(N*2^N) BFS計算量O(N^2*2^N)
void Main () {
    int N; cin>>N;
    string S; cin>>S;
    string T; cin>>T;
    ...
    while(!que.empty()){
        ...
        int empty_idx = 0;//空きマスの位置
        rep(i,str.size())if(str[i]=='.'){empty_idx=i;break;}
        rep(i,str.size()-1){
            if(str[i]=='.'||str[i+1]=='.')continue;
            string nstr = str;
            swap(nstr[i],nstr[empty_idx]);
            swap(nstr[i+1],nstr[empty_idx+1]);
            if(!dp.count(nstr)){
                dp[nstr] = dp[str]+1;
                que.push(nstr);
                }
