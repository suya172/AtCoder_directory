...
--------------------------------------------------------
*/
bool dfs(ll& H,ll& W,vs& S,vvb& seen,ll i,ll j,bool res){
    seen[i][j] = true;
    if(i==0||i==H-1||j==0||j==W-1)res=true;
    ll x[] = {1,-1,0,0};
    ll y[] = {0,0,1,-1};
    rep(k,4){
        ll ni = i + x[k];
        ...
        if(S[ni][nj]=='#'||seen[ni][nj]) continue;
        res |= dfs(H,W,S,seen,ni,nj,res);
    }
    return res;
}
void Main () {
   ...
   rep(i,H)rep(j,W){
    ...
    debug(j);
    ans++;
    if(dfs(H,W,S,seen,i,j,false))ans--;
   }
   print(ans);
} 
