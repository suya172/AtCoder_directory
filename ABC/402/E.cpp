...
void Main () {
    ...
    vector<vector<ld>> dp(X+2,vector<ld>(N+2,0));
    for(ll pos=1;pos<=N;pos++)for(ll cost=0;cost<=X;cost++){
        chmax(dp[cost][pos],dp[cost][pos-1]);
        ld hoge = 1,e = 0, hogehoge=0;
        for(ll i = 1;cost-C[pos]*i>=0;i++){
            hogehoge+=S[pos]*P[pos]*hoge;
            e+=dp[cost-C[pos]*i][pos-1]+hogehoge;
            hoge*=(1-P[pos]);
        }
        chmax(dp[cost][pos],e);
    }
    ld ans = -1;
    for(ll i=0;i<=X;i++)chmax(ans,dp[i][N]);
    print(ans);
    debug(dp);
}