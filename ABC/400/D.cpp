    ...
    while(!que.empty()){
        ...
        rep(i,4){
            ...
            if(chmin(dist[nx][ny],d)){
                if(wall)que.pb(mp(nx,ny));
                else que.pf(mp(nx,ny));
            }
            if((nnx<0)||(nnx>h-1)||(nny<0)||(nny>w-1)) continue;
            if(!(wall&&(G[nnx][nny]=='#'))) continue;
            if(chmin(dist[nnx][nny],d)){
                if(wall)que.pb(mp(nnx,nny));
                else que.pf(mp(nnx,nny));
            }
        }
    }
    ll ans = dist[c][d];
