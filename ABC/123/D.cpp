...
//     vll A(X),B(Y),C(Z);cin>>A>>B>>C;
//     sort(rall(A));sort(rall(B));sort(rall(C));
//     priority_queue<Data> que;
//     set<Data> S;
//     que.push(Data(A[0]+B[0]+C[0],vi({0,0,0})));
//     rep(k,K){
//         auto c = que.top();que.pop();
//         print(c.first);

//         if(c.second[0]+1<X){
//             auto d = Data(c.first-A[c.second[0]]+A[c.second[0]+1],vi({c.second[0]+1,c.second[1],c.second[2]}));
//             if(!S.count(d)){
//                 S.insert(d);
//                 que.push(d);
//             }
//         }
//         if(c.second[1]+1<Y){
//             auto d = Data(c.first-B[c.second[1]]+B[c.second[1]+1],vi({c.second[0],c.second[1]+1,c.second[2]}));
//             if(!S.count(d)){
//                 S.insert(d);
//                 que.push(d);
//             }
//         }
//         if(c.second[2]+1<Z){
//             auto d = Data(c.first-C[c.second[2]]+C[c.second[2]+1],vi({c.second[0],c.second[1],c.second[2]+1}));
//             if(!S.count(d)){
//                 S.insert(d);
//                 que.push(d);
//             }
...
void Main () {
    ...
    vll A(X),B(Y),C(Z);cin>>A>>B>>C;
    vll AB;
    for(auto& a:A)for(auto& b:B)AB.pb(a+b);
    RSORT(AB);
    debug(AB);
    vll ABC;
    rep(i,min(K,SIZ(AB)))rep(j,Z)ABC.pb(AB[i]+C[j]);
    RSORT(ABC);
    rep(i,K)print(ABC[i]);
}
