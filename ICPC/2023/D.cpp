...
string mkKey(int k,int t,int L,int r,const bitset<BMS> d){
    ostringstream oss;
    oss<<k<<"_"<<t<<"_"<<L<<"_"<<r<<d.to_string();
    return oss.str();
}
bool dfs(int k,int t,int L,int r,const bitset<BMS>& d){
    if((d&R)==R){
        if(r>=L*(t-k)) return true;
    }
    if(k==t) return (r==0&&((d&R)==R));
    string key=mkKey(k,t,L,r,d);
    if(M.count(key)) return M[key];
    int left = t-k;
    int maxVal=r/left;
    for(int x=L;x<=maxVal;x++){
        bitset<BMS> nd = d|(d<<x);
        if(dfs(k+1,t,x,r-x,nd)){
            M[key]=true;
            return true;
        }
    }
