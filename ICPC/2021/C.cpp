...
void solve(string S){
    ...
    // debug(G);
    // debug(V);
    um<ll,pll> memo(siz-1);
    auto f = [&](auto f,ll i,ll prev)-> pll {
        if(memo.count(i))return memo[i];
        pll res;
        if(V[i]>-1)res={V[i],V[i]};
        else if(V[i]==-1){
            res={0LL,0LL};
            for(auto& ni:G[i]){
                if(ni==prev)continue;
                res.first+=f(f,ni,i).first;
                res.second+=f(f,ni,i).second;
            }
        }else{
            vpll l;
            for(auto& ni:G[i]){
            ...
            ll ma=-1,mi=LINF;
            do{
                ll hoge=l[aa[0]].first;
                reps(i,1,aa.size())hoge-=l[aa[i]].second;
                chmax(ma,hoge);
                hoge=l[aa[0]].second;
                reps(i,1,aa.size())hoge-=l[aa[i]].first;
                chmin(mi,hoge);
    ...
    ll ans =-1;
    rep(i,siz-1){
        if(V[i]>-1)continue;//葉は根にならない
        rep(j,3){
            chmax(ans,f(f,i,-1).first);
        }
    }
    print(ans);
}
